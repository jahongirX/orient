...
$this->title = Yii::t('main', 'Update Settings');
...
{
    $lang_model = \common\models\SettingsLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\SettingsLang();
        $action = "/settings-lang/create";

    } else {

        $lang_model = \common\models\SettingsLang::findOne($lang_model->id);
        $action = "/settings-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/settings-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
